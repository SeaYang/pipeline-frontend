...
import {
  ...
  type TaskTemplateVersionStatus,
} from '@/api/taskTemplate'
import CodeEditor from '@/components/common/CodeEditor.vue'
import { convertFormat, detectFormat, type TextFormat } from '@/utils/yaml'
import { formatDateTime } from '@/utils/time'

/** 路由参数注入：当前任务模板编码（/task-template/:taskTemplateCode/versions） */
const props = defineProps<{ taskTemplateCode: string }>()
...
const versions = ref<TaskTemplateVersion[]>([])

async function fetchVersions() {
  listLoading.value = true
  try {
    versions.value = await listVersions(props.taskTemplateCode)
  } catch (e) {
...
const currentVersion = ref<TaskTemplateVersion | null>(null)
/** 当前选中的版本号（用于右侧卡片高亮） */
...
const currentStatus = computed<TaskTemplateVersionStatus | undefined>(
  () => currentVersion.value?.status ?? 'DRAFT',
)

function statusMeta(status?: TaskTemplateVersionStatus): { text: string; type: 'info' | 'success' | 'warning' } {
  switch (status) {
    case 'EFFECTIVE':
...
function applyVersion(v: TaskTemplateVersion) {
  mode.value = 'edit'
...
/** 点击右侧版本卡片：拉取详情并装入表单 */
async function handleSelectVersion(v: TaskTemplateVersion) {
  detailLoading.value = true
  try {
    const detail = await getVersionDetail(props.taskTemplateCode, v.version)
    applyVersion(detail)
  } catch (e) {
...
/**
 * 切换编辑器格式：把当前内容解析后转成目标格式。
 * 不用 v-model 绑定 radio，便于解析失败时让 radio 自动回滚到原格式。
 */
function handleFormatChange(next: TextFormat) {
  ...
  try {
    templateDetail.value = convertFormat(templateDetail.value, format.value, next)
    format.value = next
  } catch (e) {
    ElMessage.error('当前内容无法按 ' + format.value + ' 解析，已保持原格式：' + ((e as Error)?.message || '格式错误'))
    // format.value 未变 → radio 因 model-value 未变而回滚
  }
}
  ...
  try {
    const saved = isEditableDraft.value && currentVersion.value
      ? await updateVersion({
          ...
          taskTemplateCode: props.taskTemplateCode,
  ...
  <div class="version-manage" v-loading="detailLoading">
    <div class="page-header">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item :to="{ path: '/task-template' }">任务模板</el-breadcrumb-item>
        <el-breadcrumb-item>版本管理</el-breadcrumb-item>
      </el-breadcrumb>
      <el-tag size="small" type="info" class="code-tag">{{ props.taskTemplateCode }}</el-tag>
    </div>
      ...
      <div class="config-panel">
        ...
        <div class="config-form">
          ...
          <div class="form-row">
            ...
            <el-radio-group :model-value="format" @change="handleFormatChange">
          ...
          <div class="form-row form-row-editor">
            <label class="form-label">模板详情</label>
            <CodeEditor
...
/* 编辑器所在行：纵向排列、横向铺满，占满表单剩余高度；min-height:0 让其可在 flex 链中正确收缩 */
.form-row-editor {
  flex: 1;
  min-height: 0;
  flex-direction: column;
...
/* 编辑器：填满编辑器行剩余高度（贴近配置区下边缘）；monaco 宿主的绝对填充由 CodeEditor 组件负责 */
.editor-wrap {
  width: 100%;
  flex: 1;
  min-height: 200px;
